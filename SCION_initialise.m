...
function run = SCION_initialise(runcontrol)
    ...
    pars.k_Sr_sfw = 0 ;
    pars.k_Sr_sedb = pars.total_Sr_removal ;
    ...
    %%%% load INTERPSTACK
%     load( 'forcings/INTERPSTACK_2021_rotated.mat' ) ;
    ...
    %%%% Jane's plant maps
    load('forcings/NPPchanghsingian.mat')
    load('forcings/NPPinduan.mat')
    load('forcings/NPPolenekian.mat')
    load('forcings/NPPanisian.mat')
    forcings.NPP_c = NPPchanghsingian ;
    forcings.NPP_i = NPPinduan ;
    forcings.NPP_o = NPPolenekian ;
    forcings.NPP_a = NPPanisian ;
    
    %%%% Sr map
    load('forcings/Srmap.mat') ;
    forcings.Srmap = Srmap ;
    load('forcings/Srmap_noST.mat') ;
    forcings.Srmap_noST = Srmap_noST ;
...
%         pars.runstamps = INTERPSTACK.time( INTERPSTACK.time > ( pars.whenstart * 1e-6 ) ) ;
        %%%% change stamps for Jane paper
        pars.runstamps = [-253 -251 -249 -245] ;
    end
