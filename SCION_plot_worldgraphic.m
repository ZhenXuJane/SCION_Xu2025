...
for f = [1]
    ...
    ha = tight_subplot(7,length(choose_gridsubs),0,0.01,0.01) ;
...
%         end
        ...
        %%%% Nan out the ocean 
        thisfield = gridstate.NPP(:,:,gridsub);
        thisfield(gridstate.land(:,:,gridsub) == 0 ) = NaN ;
        %%%% plot NPP
        axes(ha(subnumber + 5.*length(choose_gridsubs) )) ;
        hold on
        m_proj('robinson','longitude',[-180 172.5],'latitude',[-86.6 86.6])
        h = m_pcolor(INTERPSTACK.lon - 180,INTERPSTACK.lat,thisfield) ;
        m_grid('box','on','xticklabels','','yticklabels','')
%         caxis([0 1])
        ...
        caxis([0 1500])
         ...
         %%%% Nan out the ocean 
        thisfield = gridstate.fbiota(:,:,gridsub);
        thisfield(gridstate.land(:,:,gridsub) == 0 ) = NaN ;
%   
        %%%% plot f_biota
        axes(ha(subnumber + 6.*length(choose_gridsubs) )) ;
        hold on
        m_proj('robinson','longitude',[-180 172.5],'latitude',[-86.6 86.6])
        h = m_pcolor(INTERPSTACK.lon - 180,INTERPSTACK.lat,thisfield) ;
        m_grid('box','on','xticklabels','','yticklabels','')
