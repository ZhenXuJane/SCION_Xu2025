...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%    Siberian traps degassing    %%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

CO2_ramp = 8e12 ;
CO2_input_force = interp1qr([-1e9 -251.99e6 -251.98e6 -251.56e6 -251.55e6 0 ]',[ 0 0 CO2_ramp CO2_ramp 0 0 ]',t) ;
...
%     PREPLANT = 1/ ( 4 + 3*sensparams.randminusplus4) ;
    ...
    %%%% Sr isitope composition
    Sr_input_uncertainty = 0.0003*sensparams.randminusplus4 ;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%   Jane's terrestrial biosphere   %%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


%%%%%% define NPP maps with single numbers (g/m2/yr)
NPP_past = land_past.*420 ;
NPP_future = land_future.*420 ;
Sr_past = land_past.*0.708 ;
Sr_future = land_future.*0.708 ;

%%%% Sr lithological maps
%%%% substitute in Sr map for future map before 245 Ma
if key_future_index == 12
    if t_geol < -251.9
        Sr_future = forcings.Srmap_noST ;
    end
    if t_geol >= -251.9 
        Sr_future = forcings.Srmap ;
    end
end

%%%% when past the 245 map, alter past map instead of future
if key_past_index == 12
    if t_geol < -251.9
        Sr_past = forcings.Srmap_noST ;
    end
    if t_geol >= -251.9 
        Sr_past = forcings.Srmap ;
    end
end
...
%%%% substitute in Jane's plant maps for future map before 245 Ma
if key_future_index == 12
    if t_geol < -251.9
        NPP_future = forcings.NPP_c ;
    end
    if t_geol > -251.9 && t_geol < -249.9
        NPP_future = forcings.NPP_i ;
    end
    if t_geol > -249.9 && t_geol < -246.7
        NPP_future = forcings.NPP_o ;
    end
    
    if t_geol > -246.7
        NPP_future = forcings.NPP_a ;
    end
end
...
if key_past_index == 12
    if t_geol < -251.9
        NPP_past = forcings.NPP_c ;
    end
    if t_geol > -251.9 && t_geol < -251.2
        NPP_past = forcings.NPP_i ;
    end
    if t_geol > -251.2 && t_geol < -247.2
        NPP_past = forcings.NPP_o ;
    end
    if t_geol > -247.2
        NPP_past = forcings.NPP_a ;
    end
end

%%%%%%%%%% Fossil NPP option end


%%%% Berner CO2 fertilization 
NPP_past = NPP_past .* ( ( (2*RCO2) / (1 + RCO2) )^0.4 )   ;
NPP_future = NPP_future .* ( ( (2*RCO2) / (1 + RCO2) )^0.4 )  ;


%%%%%% define NPP maps in g/yr
NPP_g_yr_past = NPP_past.*INTERPSTACK.gridarea.*1e6 ;
NPP_g_yr_future = NPP_future.*INTERPSTACK.gridarea.*1e6 ;


%%%% calculate relative mass of biosphere
NPP_present = 6e16 ;
VEG_past = sum(sum(NPP_g_yr_past))./NPP_present ;
VEG_future = sum(sum(NPP_g_yr_future))./NPP_present ;
VEG = ( VEG_past*contribution_past + VEG_future*contribution_future ) ;


%%%% calculate weathering enhancement from biota (spatial map)

% fbiota_past = 0.0025.*NPP_past + 0.15 ;
% fbiota_future = 0.0025.*NPP_future + 0.15 ;

fbiota_past = 0.002.*NPP_past + 0.25 ;
fbiota_future = 0.002.*NPP_future + 0.25 ;

% fbiota_past = 0.0012.*NPP_past + 0.5 ;
% fbiota_future = 0.0012.*NPP_future + 0.5 ;

% fbiota_past = land_past.*1 ;
% fbiota_future = land_future.*1 ;
...
% %%% kump maxed out silicate weathering option
% if t_geol > -252 && t_geol < -247
%     EPSILON_past = EPSILON_past*0.1 ;
%     EPSILON_future = EPSILON_future*0.1 ;
% end
...
%%%% CW total (including biotic effect)
CW_past = CW_per_km2_past .* GRID_AREA_km2 .* fbiota_past ;
CW_future = CW_per_km2_future .* GRID_AREA_km2 .* fbiota_future ;
...
%%%% strontium ratio of silicate weathering over space
Sr_isotope_input_past = CW_past .* Sr_past;
Sr_isotope_input_future = CW_future .* Sr_future ;

%%%% world Sr input ratio
Sr_isotope_input_past(isnan(Sr_isotope_input_past)==1) = 0 ;
Sr_isotope_input_future(isnan(Sr_isotope_input_future)==1) = 0 ;
Sr_isotope_input_sum_past = sum(sum(Sr_isotope_input_past)) ;
Sr_isotope_input_sum_future = sum(sum(Sr_isotope_input_future)) ;
Sr_isotope_input_tot = ( Sr_isotope_input_sum_past*contribution_past + Sr_isotope_input_sum_future*contribution_future ) / CW_tot;

%%% add uncertainty
if sensanal == 1
    Sr_isotope_input_tot = Sr_isotope_input_tot + Sr_input_uncertainty ;
end
...
%%%% CW total (including biotic effects)
CWcarb_past = CWcarb_per_km2_past .* GRID_AREA_km2 .* fbiota_past  ;
CWcarb_future = CWcarb_per_km2_future .* GRID_AREA_km2 .* fbiota_future  ;
%%%% world CWcarb
...
%%%% generic weathering without biotic effects - for climate dependence of oxidw, pyrw, gyps
CWgeneric_past = CWcarb_per_km2_past .* GRID_AREA_km2  ;
CWgeneric_future = CWcarb_per_km2_future .* GRID_AREA_km2  ;
%%%% world CWcarb
CWgeneric_past(isnan(CWgeneric_past)==1) = 0 ;
CWgeneric_future(isnan(CWgeneric_future)==1) = 0 ;
CWgeneric_sum_past = sum(sum(CWgeneric_past)) ;
CWgeneric_sum_future = sum(sum(CWgeneric_future)) ;
...
generic_weathering_spatial = ( CWgeneric_sum_past*contribution_past + CWgeneric_sum_future*contribution_future ) ;
...
%%%% overall weathering
silw = basw + granw ;
generic_weathering_relative = (generic_weathering_spatial/pars.k_carbw) ;

% %%% maxed out silicate weathering option
% if t_geol > -252 && t_geol < -247
%     silwmax = pars.k_silw * 0.8 ;
%     silw = min(silw, silwmax) ;
% end


%%%% oxidative weathering 
oxidw = pars.k_oxidw*generic_weathering_relative*(G/pars.G0)*((O/pars.O0)^pars.a) ;

%%%% pyrite weathering 
pyrw = pars.k_pyrw*generic_weathering_relative*(PYR/pars.PYR0)  ;

%%%% gypsum weathering 
gypw = pars.k_gypw*(GYP/pars.GYP0)*generic_weathering_relative ;
...
% %%% reverse weathering option
% if t_geol > -252 && t_geol < -247
%     mccb = carbw + silw*0.33 ;
% else
%     mccb = carbw + silw ;
% end
% 
...
%%% Carbon dioxide
dy(3) = -locb - mocb + oxidw + ocdeg + ccdeg + carbw - mccb - sfw  + reductant_input + CO2_input_force  ;
...
%%% delta_A * A
dy(16) = -locb*(  delta_locb ) -mocb*( delta_mocb ) + oxidw*delta_G + ocdeg*delta_G + ccdeg*delta_C + carbw*delta_C - mccb*delta_mccb - sfw*delta_mccb + reductant_input*-5 + CO2_input_force*d13c_CO2_input_force;
...
%%%% Ocean [Sr]
% dy(18) = Sr_granw + Sr_basw + Sr_sedw + Sr_mantle - Sr_sedb  ;
dy(18) = Sr_granw + Sr_basw + Sr_sedw + Sr_mantle - Sr_sedb  ;

%%%% Ocean [Sr]*87/86Sr
% dy(19) = Sr_granw*dSr_gran + Sr_basw*dSr_bas + Sr_sedw*delta_SSr + Sr_mantle*dSr_mantle - Sr_sedb*delta_OSr - Sr_sfw*delta_OSr ;

%%%% for spatially-summed Sr isotope values
dy(19) = (Sr_granw + Sr_basw + Sr_sedw)*Sr_isotope_input_tot + Sr_mantle*dSr_mantle - Sr_sedb*delta_OSr  ;
...
if sensanal == 0
    ...
    workingstate.Sr_isotope_input_tot(stepnumber,1) = Sr_isotope_input_tot ;
    %%%%%%%% print time
    if pars.runcontrol == -2
...
if sensanal == 0
    ...
    if pars.finishgrid == 0
        if t_geol > next_stamp || t_geol == 0
            ...
            %%%% write gridstates -- for Jane's paper take future stamp if before -245
            
            if t_geol < -245
            
                gridstate.time_myr(pars.gridstamp_number,1) = next_stamp ;
                gridstate.land(:,:,pars.gridstamp_number) = land_future ;
                gridstate.Q(:,:,pars.gridstamp_number) = Q_future ;
                gridstate.Tair(:,:,pars.gridstamp_number) = Tair_future ;
                gridstate.TOPO(:,:,pars.gridstamp_number) = TOPO_future ;
                gridstate.CW(:,:,pars.gridstamp_number) = CW_per_km2_future ; %%% t/km2/yr
                gridstate.CWcarb(:,:,pars.gridstamp_number) = CWcarb_future ;
                gridstate.EPSILON(:,:,pars.gridstamp_number) = EPSILON_future * 1e6 ; %%% t/km2/yr
                gridstate.NPP(:,:,pars.gridstamp_number) = NPP_future ;
                gridstate.fbiota(:,:,pars.gridstamp_number) = fbiota_future ;
                ...
                gridstate.fbiota(:,:,pars.gridstamp_number) = fbiota_past ;
                
            end
            

            %%%% set next boundary
            if t_geol <  pars.runstamps(end)
